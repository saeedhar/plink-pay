...
const UpdateKYBSuccess: React.FC = () => {
  const navigate = useNavigate();
  const location = useLocation();

  useEffect(() => {
    // Get KYB data from navigation state if needed
    const state = location.state as { kybData?: any };
    if (state?.kybData) {
      // Could log or use the data if needed
      console.log('KYB Data:', state.kybData);
    }
  }, [location]);
            ...
            <div className="text-center">
              ...
              <div className="text-center mb-6">
                <h2 className="text-2xl font-bold text-[#022466] mb-2">
                  KYB Updated Successfully
                </h2>
                <p className="text-gray-600">
                  Your business verification information has been successfully updated on{' '}
...
export default UpdateKYBSuccess;
